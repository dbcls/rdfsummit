...
class AssemblyReports2RDF
  ...
  REPORT_FILES = %w(assembly_summary_refseq.txt assembly_summary_genbank.txt)
  ...
  def identifier_type
      if @source == 'assembly_summary_refseq.txt'
         'refseq'
      elsif @source == 'assembly_summary_genbank.txt'
         'insdc'
      end
  end
  ...
  def output_assembly_reports id
    ...
    dir = id[0,9]
    if FileTest.exist?("#{ASSEMBLY_ROOT}/All/#{dir}/#{id}.assembly.txt")
      File.readlines("#{ASSEMBLY_ROOT}/All/#{dir}/#{id}.assembly.txt").each_with_index do |line,i|
         next if line =~/^#/
  ...
  def output_assembly_stats id
      dir = id[0,9]
      if FileTest.exist?("#{ASSEMBLY_ROOT}/All/#{dir}/#{id}.stats.txt")
        File.readlines("#{ASSEMBLY_ROOT}/All/#{dir}/#{id}.stats.txt").each_with_index do |line,i|
          next if line =~/^#/
          ...
          if unit_name == 'all' and molecule_name == 'all' and  molecule_type_loc == 'all' and sequence_type == 'all'
             #pp [statistic, value]
             output_statistic statistic, value
          end
        end
      end
  end

  def output_statistic statistic, value
      puts "\tasm:#{statistic}\t#{value} ;"
  end
  ...
  def output_prefix
      puts "@prefix rdf: <http://www.w3.org/1999/02/22-rdf-syntax-ns#> ."
      puts "@prefix rdfs: <http://www.w3.org/2000/01/rdf-schema#> ."
      #puts "@prefix obo: <http://purl.obolibrary.org/obo/> ."
      puts "@prefix sio: <http://semanticscience.org/resource/> ."
  ...
  def output_project
    ...
    #@reports.first(5).each do |project|
         ...
         #puts "<#{@project_uri}>"
         puts "["
         puts "\trdf:type\tasm:Assembly_Database_Entry ;"
         #puts "\tsio:SIO_000068\t<http://identifiers.org/#{@idtype}> ;" # sio:is-part-of
         puts "\trdf:type\t<http://identifiers.org/#{@idtype}> ;" # sio:is-part-of
         puts "\tasm:wasDerivedFrom \"#{@source}\" ;" # prov:wasDerivedFrom
  ...
  def output_pv k,v
      #p [k,v]
       ...
       when 'BioSample Accession','biosample'
          ...
          puts "\tasm:biosample\t<http://identifiers.org/biosample/#{v}> ;" if (v != '-' and  v != 'na' and v != '')
       ...
       when 'excluded_from_refseq'
          puts "\tasm:excluded_from_refseq\t#{quote(v)} ;"
       when 'Pubmed ID'
